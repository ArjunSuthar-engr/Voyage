    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-2">
        {activities.map((activity) => (
          <div
            key={activity.title}
            className="flex items-center justify-between gap-3 rounded-none border border-white/10 bg-[#1d2127] p-3 text-left transition hover:border-teal-300 hover:shadow-sm"
          >
            <div className="min-w-0 space-y-1">
              ...
              <p className="line-clamp-1 text-xs text-white/50">{activity.notes}</p>
            </div>
            <div className="flex shrink-0 items-center gap-3">
              <span className="text-sm font-medium text-white/70">{formatCurrency(activity.cost, currency)}</span>
